// Pantalla de resultado para Ejercicio 1 (simula Activity de resultado via Intent)
import React from 'react';
import { View, Text, TouchableOpacity, StyleSheet } from 'react-native';

export default function Ejercicio1Resultado({ route, navigation }) {
  const { titulo, resultado, formula, opcion } = route.params;

  const unidades = { voltaje: 'V', corriente: 'A', resistencia: 'Ohm' };

  return (
    <View style={styles.container}>
      <Text style={styles.label}>Formula aplicada:</Text>
      <Text style={styles.formula}>{formula}</Text>

      <View style={styles.card}>
        <Text style={styles.cardLabel}>{titulo} calculado:</Text>
        <Text style={styles.valor}>
          {resultado} {unidades[opcion]}
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    shadowOpacity: 0.1, shadowRadius: 4, marginBottom: 28,
  },
